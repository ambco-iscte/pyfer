...
from model.classifier.training.tensorutils import encode
...
def load(
    ...
    for (index, row) in fer.iterrows():
        ...
        emotion_index = np.argmax(fer_plus_votes)

        # Exclude Unknown or NF
        if emotion_index not in emotions.keys():
            continue
